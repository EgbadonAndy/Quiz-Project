...
['What is 5 multiply 25?', 100, 150, 125, "C"],
...
function renderQuestion()
{
	...
	{
		test.innerHTML = "<h2>You got "+correct+" of "+question.length +" questions correct(" +percent() + "%)</h2>"; //Print # correct out of 20
	...
	test.innerHTML += "<input type='radio' name='choices' value='B' checked> "+choiceB+"<br>";//Radio button
	test.innerHTML += "<input type='radio' name='choices' value='C' checked> "+choiceC+"<br><br>";//Radio button
...
{
	choices = document.getElementsByName("choices"); //create an array
for(var i=0; i<choices.length; i++)//variables i=0, when i is less than the length of the choices, increment it by one
{
	if(choices[i].checked) // if a choice is checked
	{
		choice = choices[i].value; // Take the value of that choice and put into choice
	
	}
}
if(choice == questions[pos][4])//If the value of choices is equal to sposition 4
{
	alert('Correct!');//Allert correct
